...
class ZeekMonitor(options:ZeekOptions, handler:ZeekHandler) {
   ...
   * use cases somewhat.
   */
  private val systemName:String = options.getSystemName
  ...
  private val pollingInterval: FiniteDuration =
    FiniteDuration(options.getPollingInterval, "seconds")

  private val maxBufferSize: Int =options.getMaxBufferSize
  private val postfix: String = options.getLogPostfix
  /*
  ...
  def start(): Unit = {
     ...
     */
    val folder = options.getLogFolder
