...
class FleetActor(options:FleetOptions, path:Path, handler:FleetHandler) extends Actor with ActorLogging {
  ...
  private val maxLineSize = options.getMaxLineSize
  /*
  ...
  protected def send(line:String):Unit = {
    try {
      ...
      handler.sendFileEvent(fileName=path.toFile.getName, fileElement=json)
