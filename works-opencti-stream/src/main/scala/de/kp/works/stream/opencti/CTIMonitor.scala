...
/**
 * The [CTIHandler] connects & listens to the OpenCTI
 * events stream via an SSE client and write these events
 * to the provided output handler.
 */
class CTIMonitor(options:CTIOptions, handler:CTIHandler) {
  ...
  def restart(t:Throwable): Unit = {
    ...
    println(s"[CTIMonitor] $now - Restart due to: ${t.getLocalizedMessage}")
