...
object DittoHelper {
  ...
  def getMessagingProvider(properties: java.util.Properties): MessagingProvider = {
    ...
    val endpoint = properties.getProperty(DittoNames.DITTO_ENDPOINT)
    builder.endpoint(endpoint)

    val proxyConfiguration: ProxyConfiguration = {
      
      if (
          properties.containsKey(DittoNames.DITTO_PROXY_HOST) &&
          properties.containsKey(DittoNames.DITTO_PROXY_PORT)) {
        
        val host = properties.getProperty(DittoNames.DITTO_PROXY_HOST)
        val port = properties.getProperty(DittoNames.DITTO_PROXY_PORT)
    ...
    if (
        properties.containsKey(DittoNames.DITTO_TRUSTSTORE_LOCATION) &&
        properties.contains(DittoNames.DITTO_TRUSTSTORE_PASSWORD)) {
             
        val location = properties.getProperty(DittoNames.DITTO_TRUSTSTORE_LOCATION)
        val password = properties.getProperty(DittoNames.DITTO_TRUSTSTORE_PASSWORD)
  ...
  def getAuthProvider(proxyConf:ProxyConfiguration, properties: java.util.Properties):AuthenticationProvider[WebSocket] = {
    ...
    if (properties.containsKey(DittoNames.DITTO_USER) && properties.containsKey(DittoNames.DITTO_PASS)) {
      
      /** BASIC AUTHENTICATION **/
      
      val user = properties.getProperty(DittoNames.DITTO_USER)
      val pass = properties.getProperty(DittoNames.DITTO_PASS)
      ...
      try {
        ...
        val scopes = {
          ...
          val tokens = properties.getProperty(DittoNames.DITTO_OAUTH_SCOPES).split(",")
        ...
        val oAuthConf = ClientCredentialsAuthenticationConfiguration.newBuilder()
          .clientId(DittoNames.DITTO_OAUTH_CLIENT_ID)
          .clientSecret(DittoNames.DITTO_OAUTH_CLIENT_SECRET)
          .scopes(scopes)
          .tokenEndpoint(DittoNames.DITTO_OAUTH_TOKEN_ENDPOINT)
