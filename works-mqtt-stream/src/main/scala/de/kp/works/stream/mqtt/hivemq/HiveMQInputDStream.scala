...
class HiveMQReceiver(
  ...
  private val hiveOptions = new HiveOptions(properties)
  /*
   * The HiveMQ clients support a single topic; the receiver interface,
   * however, accept a list of topics to increase use flexibility
   */
  ...
  private def getMqttTopic: String = {
    ...
    val mqttTopics = hiveOptions.getTopics
  ...
  def onStart() {
     ...
     */
    val version = hiveOptions.getVersion
    if (version == 3)
        listenToMqtt3()
